...
public class Config {
	 ...
	 * 在浏览器访问wsdl文件地址为 http://localhost:8088/agent/RmsAgent?wsdl
	 * 用jws wsimport.exe生成客户端代码命令，新建一个文件夹进入cmd输入命令：
	 * wsimport -s . http://localhost:8088/agent/RmsAgent?wsdl
	 * @return
	 */
	@Bean
	public SimpleJaxWsServiceExporter simpleJaxWsServiceExporter(){
		SimpleJaxWsServiceExporter sJaxWs = new SimpleJaxWsServiceExporter();
		sJaxWs.setBaseAddress("http://0.0.0.0:8088/agent/");
		return sJaxWs;
	}
}
