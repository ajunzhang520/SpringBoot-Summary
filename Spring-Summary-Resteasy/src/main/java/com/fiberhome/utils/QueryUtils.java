...
public class QueryUtils {
	...
	public static Map<String, String> toSingleValueMap(MultivaluedMap<String, String> query) {
		Map<String, String> map = new HashMap<>(query.size());
		query.forEach((key, value) -> {
			map.put(key, query.getFirst(key));
		});
		return map;
	}

	public static QueryFilter parseMultiQuery(MultivaluedMap<String, String> query) throws Exception {
		...
		query.forEach((key, value) -> {
			if ("Q".equals(key)) {
				...
				queryFilter.addParams(key, StringUtils.trimWhitespace(value.get(0)));
			}
		});
